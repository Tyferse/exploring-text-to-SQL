...
import sys
csv.field_size_limit((2 << 30) - 1) # sys.maxsize)
...
class REFORCE:
                    ...
                    if sqls != []:
                        ...
                        try:
                            logger.info("[Corrected other sqls]\nuser:\n" 
                                + self.chat_session.messages[-2]['content']
                                + "\nassistant:\n"
    ...
    def self_correct(self, sql, error, logger, simplify=False):
        ...
        try:
            logger.info("[Explore and Correct SQL]\nuser:\n" 
                + self.chat_session_pre.messages[-2]['content']
                + "\nassistant:\n"
                + self.chat_session_pre.messages[-1]['content']
                + "\n[Explore and Correct SQL]"
            )
        except IndexError:
            pass
    ...
    def format_answer(self, task, chat_session, logger):
    ...
    def exploration(self, task, table_struct, table_info, logger):
        ...
        max_try = self.max_try
        while max_try > 0:
            logger.info("itercount: " + str(self.max_try - max_try))
    ...
    def self_refine(self, args, logger, question, format_csv, table_struct, table_info, response_pre_txt, pre_info, csv_save_path, sql_save_path, task=None):
        ...
        while itercount < args.max_iter:
            ...
            if executed_result == '0':
                ...
                df_csv = pd.read_csv(csv_buffer, encoding='utf-8').fillna("")
    ...
    def model_vote(self, result, sql_paths, search_directory, args, table_info, task, logger):
    ...
    def vote_result(self, search_directory, args, sql_paths, table_info, task, logger):
        # filter answer
        result = {}
        result_name = {}
        result_all = {}
        ...
        if not result:
            if not result_all and not all_values:
                ...
                # print(result_all)
                self.model_vote(result_all, sql_paths, search_directory, args, table_info, task, logger)
            elif args.final_choose:
                csv_idx = 0
                while not os.path.exists(all_values[csv_idx].replace(".csv", ".sql")) and csv_idx < len(all_values):
                    csv_idx += 1

                csv_pth = all_values[csv_idx]
        ...
        if has_tie:
            ...
            if args.model_vote:
                self.model_vote(result, sql_paths, search_directory, args, table_info, task, logger)
                return
            if not args.random_vote_for_tie:
                print(f"{search_directory} has_tie {sorted_dict}, return")
                return

        # pint(search_directory, first_key, self.complete_sql_save_path)
        first_key = first_key if not first_key.startswith(search_directory) else first_key[len(search_directory) + 1:]
        try:
            shutil.copy2(os.path.join(
                search_directory, 
                first_key
            ), self.complete_sql_save_path)
