...
# fi
env_python="/usr/local/envs/reforce/bin/python"

# ${env_python} spider_agent_setup_${TASK}.py --example_folder examples_${TASK}

# Reconstruct data
# ${env_python} reconstruct_data.py \
...
# ${env_python} schema_linking.py \
#     --task $TASK \
#     --db_path examples_${TASK} \
#     --linked_json_pth ../../data/linked_${TASK}_tmp0.json \
#     --reduce_col \
#     --gold_tb_pth "../gold-tables/spider2-lite-gold-tables.jsonl"
...
${env_python} eval.py --log_folder $OUTPUT_PATH --task $TASK
...
${env_python} get_metadata.py --result_path $OUTPUT_PATH --output_path output/${API}-${TASK}-csv-${TIMESTAMP}
${env_python} get_metadata.py --result_path $OUTPUT_PATH --output_path output/${API}-${TASK}-sql-${TIMESTAMP} --file_type sql
cd ../../spider2-${TASK}/evaluation_suite
${env_python} evaluate.py --mode exec_result --result_dir ../../methods/ReFoRCE/output/${API}-${TASK}-csv-${TIMESTAMP}