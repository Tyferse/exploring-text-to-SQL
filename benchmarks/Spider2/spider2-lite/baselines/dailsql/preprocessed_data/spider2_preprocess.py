...
def process_table_json(args, proj_dir):
    ...
    # os.makedirs(osp.join(proj_dir, f'preprocessed_data/{args.dev}/tables_preprocessed'), exist_ok=True)
    ...
    # for i in range(len(db_stats_list) // 15):
    #     with open(osp.join(proj_dir, f"preprocessed_data/{args.dev}/tables_preprocessed/{i + 1:4>0}.json"), "w", encoding="utf-8") as json_file:
    #         json.dump(db_stats_list[15*i : 15*(i + 1)], json_file, indent=4, ensure_ascii=False)
...
def process_dev_json(args, proj_dir):
    ...
    #         table_json.append(json.load(json_file))            
            
    #         try:
    #             while True:
    #                 nan_replaced.append(json_file.readline())
    ...
    #             nan_replaced = list(
    #                 executor.map(lambda line: line.replace('NaN,', 'null,') + '\n' 
    #                              if 'NaN,' in line else line + '\n', 
    #                              json_file.readlines()))
    ...
    # with open(osp.join(proj_dir, f"preprocessed_data/{args.dev}/tables_preprocessed.json"), "r", encoding="utf-8-sig") as json_file:    
    #     parser = ijson.items(json_file, 'item', ensure_ascii=False)
    #     for item in parser:
