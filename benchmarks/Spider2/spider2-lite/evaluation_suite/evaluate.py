...
def get_bigquery_sql_result(sql_query: str, is_save: bool, save_dir=None, file_name: str = "result.csv", instance_id: str = None):
    ...
    os.environ.setdefault("GOOGLE_APPLICATION_CREDENTIALS", os.path.dirname(__file__) + "/bigquery_credential.json")
...
def evaluate_single_sql_instance(
    ...
    try:
        pred_sql_path = Path(pred_result_dir) / f"{instance_id}{pred_result_postfix}.sql"
...
def evaluate_spider2sql(args, temp_dir: Path):
    ...
    pred_ids = []
    if mode == "sql":
        pred_ids = [Path(file).stem[:-2] for file in os.listdir(pred_result_dir) if file.endswith(".sql")]
