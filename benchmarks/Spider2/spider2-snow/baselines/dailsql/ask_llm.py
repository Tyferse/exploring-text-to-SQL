...
from llm.openrouter import init_client, ask_llm
from utils.enums import LLM
from utils.post_process import process_duplication, get_sqls


# load_dotenv("/media/sf_UCode/ttsql/.env")
load_dotenv("../../../../../.env")
openrouter_api_key = os.environ.get('UPSTAGE_API_KEY')


def process_batch(batch, submit_folder, db_ids, args, api_key, i):
    """Function to process each batch in parallel."""

    # cost recorded
    os.makedirs(os.path.join(submit_folder, "../cost"), exist_ok=True)
    with (open(
        os.path.join(submit_folder, "../cost", f"{batch['instance_id'][0]}.json"),
         "w") as submit_file):
    ...
    try:
        res = ask_llm(client, args.model, batch["prompt"], args.n, args.thinking)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--processes", type=int, default=20)  # New argument for specifying the number of processes
    ...
    if args.override:
        ...
        pred_ids = {file.split(".")[0].split("@")[0]
                    for file in os.listdir(submit_folder) if file.endswith(".sql")}
        # pred_ids = set(pred_ids)
        
    questions_json = json.load(open(os.path.join(args.question, QUESTION_FILE), "r"))
    questions = [{"prompt": item["prompt"], "instance_id": item["instance_id"]}
    ...
    with ThreadPool(processes=args.processes) as pool:
        with tqdm(total=len(question_loader)) as pbar:
            tasks = ((batch, submit_folder, db_ids, args, openrouter_api_key, i) 
                    for i, batch in enumerate(question_loader))
            
            for _ in pool.starmap(process_batch, tasks):
                pbar.update(1)
