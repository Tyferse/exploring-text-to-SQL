...
def execute(question, table_info, args, csv_save_path, log_save_path, sql_save_path, search_directory, format_csv, sql_data):
    ...
    if os.path.exists(os.path.join(search_directory, sql_save_path)) and not os.path.exists(os.path.join(search_directory, csv_save_path)) and args.revote:
        with open(os.path.join(search_directory, sql_save_path), encoding='utf-8') as f:
            sql = f.read()
...
def process_sql_data(sql_data):
    ...
    if len(table_info) > 100000:
        print(f"Table info len: {len(table_info)}, return")

    # Create logs for outer chat data
    log_file_path = os.path.join(search_directory, agent_format.log_save_name)
    logger = initialize_logger(log_file_path)
    ...
    if args.do_format_restriction:
        if args.use_gold_format:
            ...
            with open(csv_pth, encoding='utf-8') as f:
                format_csv = "```sql\n"+f.read().split("\n")[0]+"\n```"
        else:
            ...
            format_csv = agent_format.format_answer(question, chat_session_format, logger)
    else:
        format_csv = None
    ...
    if args.do_vote:
        ...
        if "result.sql" not in os.listdir(search_directory):
            if any(file.endswith('.sql') for file in os.listdir(search_directory) if os.path.isfile(os.path.join(search_directory, file))):
                ...
                agent_format.vote_result(search_directory, args, sql_paths, table_info, question, logger)
            else:
...
if __name__ == '__main__':
    ...
    if args.omnisql_format_pth:
        if args.subtask == "sqlite":
            with open(args.omnisql_format_pth, encoding='utf-8') as f:
                data = json.load(f)
            dictionaries = []
            task_dict = {}
            full_tb_info = {}
    ...
    # Temporary problem with BigQuery
    # dictionaries = [inst for inst in dictionaries 
                    # if not (inst.startswith(("bq", 'ga', 'local', 'sf0')) or (inst.startswith("sf_bq") and int(inst[-3:]) <= 458))]
                    # if not (inst.startswith("bq") and int(inst[-3:]) <= 328)]
    # for inst in tuple(task_dict.keys()):
    #     if inst.startswith("bq") and int(inst[-3:]) <= 328:
        # if inst.startswith(("bq", 'ga', 'local', 'sf0')) or (inst.startswith("sf_bq") and int(inst[-3:]) <= 458):
            # del task_dict[inst]

    # folder = "output\solar-pro3-lite-log"
    # all_examples = os.listdir(folder)
    # rerun_examples = []
    ...
    #         if len(open(os.path.join(folder, inst, file), encoding='utf-8').readlines()) < 1:
    #             print(inst)
    #             rerun_examples.append(inst)
    #             shutil.rmtree(os.path.join(folder, inst))
    #             break
    
    # dictionaries = [inst for inst in dictionaries if inst in rerun_examples or inst not in all_examples]
    # for inst in tuple(task_dict.keys()):
    #     if inst not in rerun_examples and inst in all_examples:
    #         del task_dict[inst]
