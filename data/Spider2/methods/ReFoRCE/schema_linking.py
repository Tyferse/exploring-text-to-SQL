...
def ask_model_sl(example_path, json_save_pth):
    ...
    def process_example(ex_id):
        ...
        chat_session = GPTChat(azure=False, model="solar-pro3", temperature=0)
        result = ask_model_sl_(tb_info, task, chat_session, os.path.join(example_path, ex_id, "log.log"))
        return ex_id, result
        ...
        with open(log_path, 'r', encoding='utf-8') as lf:
            for line in lf.readlines():
                if re.match(
                    ...
                    line.strip()
                ) and not line.strip().endswith("itercount: 0"):
                    is_sl = True
                    log_lines.append([])
                elif is_sl:
                    if line.startswith(("user:", "assistant:")):
                        log_lines[-1].append("")
                    elif line.startswith("[Schema linking]"):
                        is_sl = False
                    else:
                        log_lines[-1][-1] += line.strip() + '\n'
            

    print("Tables ", len(tbs), "| lines", len(log_lines))
    logger = initialize_logger(log_path)
    logger.info("itercount: 0")

    ex_tb_idx = 0
    for tb in tbs:
        if ex_tb_idx < len(log_lines) and tb in log_lines[ex_tb_idx][0] and len(log_lines[ex_tb_idx]) == 2:
            resp = log_lines[ex_tb_idx][1]
            data = json.loads(resp[resp.find('```json\n') + len('```json\n'):resp.rfind('\n```')])
            data["table name"] = re.search(r'^Table full name:\s*(.+)$', tb, re.MULTILINE).group(1)
            ex_tb_idx += 1

            try:
                logger.info("[Schema linking]\nuser:\n" 
                    + log_lines[ex_tb_idx][0]
                    + "\nassistant:\n"
                    + log_lines[ex_tb_idx][1]
                    + "\n[Schema linking]"
                )
            except IndexError:
                pass
