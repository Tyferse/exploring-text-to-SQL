...
def check_length(existing_prompt, new, future_prompt):
    return len(existing_prompt + new + future_prompt) <= context_len
...
def schema_linking_prompt_maker(test_sample_text, db_names, spider_schema, spider_foreign):
  ...
  prompt_len = (len(instruction) + len(schema_linking_prompt) 
    + len(all_foreign_keys) + len(test_sample_text) + 40)
  prompt = instruction + schema_linking_prompt + (
    all_fields if context_len - len(all_fields) > prompt_len
    else all_fields[:(context_len - prompt_len)] if prompt_len < context_len else "")
  prompt += all_foreign_keys + 'Q: "' + test_sample_text + """"\nA: Let’s think step by step."""
  return prompt
...
def llm_generation(model, prompt, n=1, think=False):
  global client
  try:
    print('>>>model:', model)
...
def llm_debug(model, prompt, n=1):
  global client
  try:  
    print('>>>model:', model)
    if model == "openai/gpt-oss-20b:free":
...
def multi_processing_process_row(index, row, args, output_dir, spider_schema, spider_primary, spider_foreign):
    ...
    for idx, sql in enumerate(SQL_list):
        ...
        while debugged_SQL is None:
            try:
                res = llm_debug(args.model, debuger(row['question'], row['db_id'], sql, spider_schema, spider_primary, spider_foreign))
                assert len(res['response']) == 1
...
if __name__ == '__main__':
    ...
    with ThreadPool(processes=args.processes) as pool:
        with tqdm(total=val_df.shape[0]) as pbar:
            ...
            # Для ThreadPool используем обычный map/starmap
